...
@Component({
    selector: 'pcosingle-demand',
    providers: [DemandService, MedicineService, TenderService, FinancialBiddingService],
    templateUrl: 'pcosingledemand.html',
    styleUrls: ['pcosingledemand.css']
})

export class pcosingledemand {
  
  selectedIndex: any = null;
  selectedTender: any = null;
  selectedTenderMeds: any = null;
  ...
  ngOnInit(){
    ...
    this.busyA = this._demandService.getTenderDemands(this.selectedTender)
        .subscribe(demands => {
          ...
          // this.tenderMedi = medicines
          console.log("LIST OF CONSOLIDATED DEMANDS:-----", this.tenderMedi);
          ...
          this.selectedTenderMeds = _.uniqBy(this.tenderMedi, '_id');
          console.log("UNIQUE _________________ LIST OF CONSOLIDATED DEMANDS:-----", this.selectedTenderMeds);
      });
  }
  // addActive(i, tender){
